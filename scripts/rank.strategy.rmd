...
```{r data read in }
setwd("analysis path")
genetab = read.csv("./data/genetab.csv")
misrevel = readRDS("./data/misrevel.RDS")
lppv = read.table("./data/REVEL_local_posteriors.txt", header = TRUE)
gnomadmis = read.table("./data/gnomad.mis.revel.tab",header = F) %>%
...
g2pubcnt = read.table("./data/g2pubcnt.wID", header = 1, sep = "\t")
...
```{r}
geneWassay = read.csv("./data/geneWassay.csv")
...
pdf(file = "./Plots/motive_b.pdf", width = 12, height = 4) 
...
sum(combo.out == FALSE)
save(combo.out, file="./new.combo.RData")
save(combo.score, file="./new.combo.score.RData")
save(weights, file = "./new.weights.RData")
```
...
pdf(file = "./Plots/Compare_combo.pdf", 
    width = 12,
    height = 6) 
...
vareff_tab = read.csv("./data/complete_prio_gene_sysVarEffMap_paper.csv")
...
pdf(file = "./Plots/FINAL.Compare_Friz.pdf", width = 12,height = 6)
...
#load("./Plots/new.combo.RData")
#load("./Plots/new.combo.score.RData")
#load("./Plots/new.weights.RData")
topgs = names(sort(combo.score[,which.max(combo.out)], decreasing = T)) %>% head(length(FINAL.assayg))
intersect(topgs, FINAL.assayg)
compgs = data.frame(combogs = topgs, assaygs = FINAL.assayg)
...
pdf(file = "./Plots/new.violA2.pdf", width = 13,height = 6) 
...
pdf(file = "./Plots/new.violB2.pdf", width = 13,height = 6) 
...
pdf(file = "./Plots/new.violC2.pdf", width = 12,height = 6)
...
```{r}
library(ggpubr)
pdf(file = "./Plots/FINAL.combo.viol.pdf", width = 13,height = 10)
...
```{r}
write.table(final.geneset, file= "./background.txt", row.names = F, col.names = F, quote = F)
write.table(togs100, file = "./combo100.txt", row.names = F, col.names = F, quote = F)
write.table(vareff_tab$DAIS, file = "./DAIS.txt", row.names = F, col.names = F, quote = F)
...
print(tab,file="./Plots/new.table1.tex",append=T,table.placement = "h",
